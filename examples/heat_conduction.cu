...
// Code for Thermal Conduction of Silver.
// Inspired by the OpenAAC implementation at:
...
__global__ void step_kernel_mod(int ni, int nj, float fact, float* temp_in, float* temp_out) {
    ...
    for (; i < ni - 1 && j < nj - 1 && i > 0 && j > 0; i += stride_row, j += stride_col) {
...
int main() {
    ...
    // Fixed size blocks means we stride over the whole matrix 
    // by the total number of elements in the particular dimension 
    // else each thread is responsible for computing one element of the matrix.
    dim3 number_of_blocks(32, 32, 1);
    // uncomment the following line for variable sized blocks to maximize occupancy
    // number_of_blocks((nj / threads_per_block.x) + 1, (ni / threads_per_block.y) + 1, 1);
